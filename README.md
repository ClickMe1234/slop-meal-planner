...
Current release: **1.4.0 Authentik test**. See the [latest release notes](#changelog) or the
...
- Open the current local day in Week view, link new planning cycles correctly,
  and keep household names alphabetical.
- Regenerate shopping lists cleanly for each plan while preserving pantry
  subtraction, and start recipe-link imports with an empty URL.

### 1.1.1 - 2026-07-26

- Restore live barcode scanning by keeping ZXing attached to the active camera
  video element, and recognise rotated barcodes in uploaded photos.
...
4. In Unraid Apps, choose **Add Container** and use the exact values in
   ...
   `ghcr.io/clickme1234/slop-meal-planner:authentiktest` as Repository, `Bridge` as
...
- Optional Authentik proxy/outpost or native OIDC authentication with local
  account linking, mode-isolated sessions, provider logout, and back-channel
  revocation. See the [Authentik setup guide](docs/authentik.md).

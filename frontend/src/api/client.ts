...
export const api = {
  ...
  confirmShoppingPantryMatch: (listId: string, itemId: string, payload: { expected_version: number; pantry_lot_id: string; decision: 'match' | 'reject' }) => request<BackendShoppingPantryMatchResult>(`/shopping-lists/${listId}/items/${itemId}/pantry-match`, { method: 'POST', body: JSON.stringify(payload) }),

...
export function OnboardingPage() {
  ...
  const [methodViewPreference, setMethodViewPreference] = useState<MethodViewPreference>('summary')
  ...
  return <div className="onboarding-page"><header className="onboarding-header"><div className="brand"><div className="brand-mark"><Heart size={20} fill="currentColor"/></div><div><strong>Slop</strong><span>meal planner</span></div></div><span>Setup {step+1} of {steps.length}</span></header><div className="onboarding-progress"><ProgressBar value={(step+1)/steps.length*100}/><ol>{steps.map((name,index)=><li key={name} className={index<=step?'active':''}><span>{index<step?<Check size={14}/>:index+1}</span>{name}</li>)}</ol></div>
    <main className="onboarding-main"><div className="onboarding-copy"><p className="eyebrow">{steps[step]}</p><h1>{['Who are we planning for?','Connect ingredient search','Set nutrition targets','Shape each day','Food that works for you','Words & cooking view','What is already at home?'][step]}</h1><p>{['Add everyone sharing meals. Each person gets their own portions and nutrition target.','A free USDA key gives your household reliable ingredient and nutrition search.','Choose calories or macros for every person. Targets stay separate and can be changed later.','Allocate each person’s target across the meals they normally eat.','Hard exclusions are always respected; preferences help rank good options.','Choose the ingredient names, cooking view and measurements that feel natural.','A quick pantry start makes your first shopping list more useful.'][step]}</p></div>
      <Card className="onboarding-card">
        ...
        {step===5&&<div className="form-stack"><p className="muted">These defaults shape recipe cards and planned-batch methods. Every method can still be switched while cooking.</p><label>Ingredient language<Segmented value={ingredientLocale} onChange={setIngredientLocale} label="Ingredient language" options={[{value:'uk',label:'British · courgette'},{value:'us',label:'American · zucchini'}]}/></label><div className="ingredient-language-example"><span>Your choice</span><strong>{ingredientLocale==='uk'?'courgette · aubergine · coriander':'zucchini · eggplant · cilantro'}</strong></div><label>Default method view<Segmented value={methodViewPreference} onChange={setMethodViewPreference} label="Default cooking method view" options={[{value:'summary',label:'Summary'},{value:'table',label:'Flow table'},{value:'written',label:'Written'}]}/></label><label>Measurements<Segmented value={measurementSystem} onChange={setMeasurementSystem} label="Method measurement system" options={[{value:'source',label:'As written'},{value:'metric',label:'Metric'},{value:'us',label:'US customary'}]}/></label><p className="muted">A search for either ingredient name checks both Good Food and Allrecipes automatically.</p></div>}

...
export function PantryPage() {
  ...
  const pantry: PantryItem[] = isDemoMode ? demoPantry : (pantryQuery.data ?? []).map(item => ({
    id: item.id,
  ...
  return <div className="page">
    ...
    <div className="pantry-list">{items.map(item => { const usable = item.quantity - item.reserved; const width = item.quantity > 0 ? Math.max(0, usable / item.quantity * 100) : 0; return <Card key={item.id} className="pantry-item"><div className="pantry-icon">{item.name.slice(0,1)}</div><div className="pantry-name"><strong>{item.name}</strong><span>{item.category}</span></div><div className="stock-meter"><div><span>Usable</span><strong>{usable} {item.unit}</strong></div><div className="macro-track"><span className="macro-fill macro-fill--green" style={{width:`${width}%`}}/></div><small>{item.reserved} {item.unit} reserved</small></div><div className="pantry-tags">{item.expires && <Badge tone="warning">Use by {item.expires}</Badge>}{item.staple && <Badge>Staple</Badge>}</div><Button variant="ghost">Edit</Button></Card>})}</div>
  </div>
}

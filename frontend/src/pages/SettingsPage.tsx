...
import { api, ApiError, type BackendRestriction } from '../api/client'
...
export function PreferenceSettings() {
  ...
  return <SettingsLayout><div className="settings-heading"><div><h2>Preferences & restrictions</h2><p>Allergies and exclusions are hard rules. Preferences only influence recipe ranking.</p></div><Badge tone="green">Saved automatically</Badge></div>{error&&<Notice tone="warning" title="Could not save">{error}</Notice>}<Card className="settings-section"><label>Household member<select value={selectedMemberId} onChange={event=>setMemberId(event.target.value)}>{members.data?.map(member=><option key={member.id} value={member.id}>{member.name}</option>)}</select></label></Card><RestrictionSection title="Household allergies" label="Strictly exclude" placeholder="e.g. peanuts" kind="allergy" tone="danger" items={items.filter(item=>item.kind==='allergy')} disabled={saving} onAdd={add} onRemove={remove}/><RestrictionSection title="Ingredient exclusions" label="Never use" placeholder="e.g. shellfish" kind="exclude" tone="danger" items={items.filter(item=>item.kind==='exclude')} disabled={saving} onAdd={add} onRemove={remove}/><RestrictionSection title="Food preferences" label="Prefer" placeholder="e.g. curries or traybakes" kind="prefer" tone="warm" items={items.filter(item=>item.kind==='prefer')} disabled={saving} onAdd={add} onRemove={remove}/><RestrictionSection title="Dislikes" label="Prefer not to use" placeholder="e.g. olives" kind="dislike" items={items.filter(item=>item.kind==='dislike')} disabled={saving} onAdd={add} onRemove={remove}/></SettingsLayout>
}
